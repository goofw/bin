...
(*.[0-9]*)
	...
	if test ! -f "/mnt/build/src/ncurses-6.5/man_alias.sed" ; then
cat >>/mnt/build/src/ncurses-6.5/man_alias.sed <<-CF_EOF2
		s,@DATADIR@,$datadir,g
...
s,@TSET@,tset,g
CF_EOF2
		echo "...made /mnt/build/src/ncurses-6.5/man_alias.sed"
	fi
	...
	aliases=`sed -f "$top_srcdir/man/manlinks.sed" "$cf_full_alias" |sed -f "/mnt/build/src/ncurses-6.5/man_alias.sed" | sort -u; test "$cf_part_alias" = "$nCurses" && echo curses`
	cf_target=`grep "^$cf_source" /mnt/build/src/ncurses-6.5/man/man_db.renames | mawk '{print $2}'`
	...
	sed	-f "/mnt/build/src/ncurses-6.5/man_alias.sed" \
		< "$i" | sed -f /mnt/build/src/ncurses-6.5/edit_man.sed >$TMP
